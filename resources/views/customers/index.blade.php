            ...
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-2">
                    ...
                    <select name="status_bayar" class="form-select form-select-sm">
                        ...
                        <option value="Sdh Bayar" {{ request('status_bayar') == 'Sdh Bayar' ? 'selected' : '' }}>
                            Sudah Bayar
                        </option>
                        <option value="Blm Bayar" {{ request('status_bayar') == 'Blm Bayar' ? 'selected' : '' }}>
                            Belum Bayar
                        </option>
                    </select>
                </div>
                    ...
                    <tbody>
                        @forelse($customers as $index => $customer)
                            <tr>
                                ...
                                <td>
                                    {{ $customer->agency_psb ?: ($customer->agency ?: '-') }}
                                </td>

                                <td>
                                    {{ $customer->sales_agency ?: ($customer->sales ?: '-') }}
                                </td>
                                ...
                                <td>
                                    @php
                                        $badgeClass = $customer->status_bayar == 'Sdh Bayar'
                                            ? 'success'
                                            : 'danger';

                                        $statusText = $customer->status_bayar == 'Sdh Bayar'
                                            ? 'Sudah Bayar'
                                            : 'Belum Bayar';
                                    @endphp
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        ...
                                        @if($customer->ssl_file)
                                            <a
                                                href="{{ route('customers.download-ssl', $customer->snd) }}"
